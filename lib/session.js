...
function Session(data, store, sockets, rawSockets) {
  ...
  store.socketQueue.once(this.sid, listener);
  setTimeout(function(){
    store.socketQueue.removeListener(this.sid, listener);
    // console.log('>>>>>>>>>>> listenerCount: ' + store.socketQueue.listenerCount(this.sid));
  }, 60000)
}

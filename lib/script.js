...
function Script(src, path) {
    this.path = path;
  try {
    this.compiled = vm.createScript('(function() {try{' + src + '\n}catch(e){console.error("Script Catch Error:", e, e.stack);throw e;}}).call(_this)', path);
  } catch(ex) {
    console.error('Script: ', path, ex.stack );
    this.error = ex;
  }
}
...
Script.prototype.run = function (ctx, domain, fn) {
  ...
  function done(err) {
    ...
    if (err){
        console.error('Script.run Error: ', self.path, err.stack || err);
    }

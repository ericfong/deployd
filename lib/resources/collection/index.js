...
// collection.events.PreGet.run(ctx, domain, function (err) {
...
Collection.prototype.remove = function (ctx, fn) {
  ...
  store.find(sanitizedQuery, function (err, result) {
    ...
    if(result && collection.shouldRunEvent(collection.events.Delete, ctx)) {
        ...
        // XXX: if id : { $in : [] }  => result is array
        var wait = new Wait();
        result = Array.isArray(result) ? result : [result];
        var err = null;

        result.forEach(function(ret){
            var domain = createDomain(ret, errors);

            // XXX: also add query to domain, so the delete know how to work
            domain.query = query;

            domain['this'] = domain.data = ret;
            collection.events.Delete.run(ctx, domain, wait(function(error){

...
Collection.events  = ['PreGet', 'Get', 'Validate', 'Post', 'Put', 'Delete'];
...
Collection.prototype.sanitize = function (body) {
  ...
  keys.forEach(function (key) {
    ...
    // XXX: Hacks
    for (var key in body) {
        if (key === '$limitRecursion') continue;
        if (key === '$skipEvents') continue;

        // XXX: allow mongodb special operation
        // FIXME: should maintain a white list instead of all
        if (SanitizeOpsWhitelist.indexOf(key) >= 0) {
            sanitized[key] = body[key];
            continue;
        }

        // XXX: Allow mixin nested doc
        var prefixIndex = key.indexOf('.');
        if (prefixIndex > 0) {
            var prefix = key.substr(0, prefixIndex);
...
var SanitizeOpsWhitelist = ['$push', '$addToSet', '$pop', '$pullAll', '$pull', '$pushAll', '$each', '$position', '$slice', '$sort'];
...
Collection.prototype.find = function (ctx, fn) {
  ...
  function done(err, result) {
    ...
    // XXX: add a pre-get Event
    if (collection.shouldRunEvent(collection.events.PreGet, ctx)) {
        var domain = createDomain(sanitizedQuery, errors);
        domain.query = query;
        collection.events.PreGet.run(ctx, domain, doFind);
    } else {
        doFind();
    }
...
Collection.prototype.remove = function (ctx, fn) {
  ...
  store.find(sanitizedQuery, function (err, result) {
    ...
    if(result && collection.shouldRunEvent(collection.events.Delete, ctx)) {
      var domain = createDomain(result, errors);

        // XXX: also add query to domain, so the delete know how to work
        domain.query = query;
...
// XXX: post if not exists
if (item.$postIfNotExists) {
    //console.log('PUT__', sanitizedQuery, item);
    item.id = id;
    ...
    return;
      ...
      // XXX allow mixin
      // FIXME: onValidate onPut Event don't know the key is changed
      Object.keys(item).forEach(function (k) {
        var prefixIndex = k.indexOf('.');
        if (prefixIndex > 0) {
            var prefix = k.substr(0, prefixIndex);
            if (prefix in item) {
                delete item[prefix];
            }
        }
  ...
  function post() {
    ...
    // XXX: open for post by specify id
